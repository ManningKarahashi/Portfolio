...
function processCommits(data) {
    return d3
        ...
        .map(([commit, lines]) => {
            ...
            let ret = {
                id: commit,
                url: 'https://github.com/ManningKarahashi/Portfolio/commit/' + commit,
                author,
                date,
                time,
                timezone,
...
function renderTooltipContent(commit) {
    ...
    document.getElementById('commit-time-display').textContent = commit.time;
...
function updateFileDisplay(filteredCommits) {
    ...
    filesContainer.select('dt > code').html(
        (d) =>
            `${d.name}<small style="display:block; font-size:0.75em; opacity:0.6;">${d.lines.length} lines</small>`,
    );
